...
class SeewoAgent:
    ...
    def __init__(self, config_path="config.yaml"):
        ...
        # Components
        self.lock_window = LockScreenWindow(agent_key=self.agent_key)
    ...
    async def _handle_command(self, message):
        ...
        try:
            if action == "lock_screen":
                image_paths = self.image_sync.get_all_images()
                logger.info(f"锁屏图片数量: {len(image_paths)}")
                self.lock_window.show(image_paths if image_paths else None)
                result["message"] = "锁屏成功"
                ...
                if duration:
                    ...
                    asyncio.get_event_loop().call_later(
                        duration * 60,
                        lambda: self.lock_window.show(self.image_sync.get_all_images())
                    )
